...
if (isset($_POST['mudar_status_completo'])) {
    try {
        ...
        if ($novo_status == 'entrevista' && !empty($_POST['data_entrevista'])) {
            ...
            $links_agenda = "<br><hr><p><strong>Adicione à agenda:</strong> <a href='{$links_cal['google']}'>Google</a> | <a href='{$links_cal['outlook']}'>Outlook</a></p>";
        }
...
<body>
    ...
    <div class="main-content">
        ...
        <?php if($pagina == 'banco_testes'): 
            ...
            if($view_id): 
                $teste = $pdo->query("SELECT * FROM banco_testes WHERE id=$view_id")->fetch();
                $questoes = $pdo->query("SELECT * FROM questoes_teste WHERE banco_teste_id=$view_id")->fetchAll();
        ?>
        ...
        <?php else: $lista = $pdo->query("SELECT * FROM banco_testes ORDER BY id DESC")->fetchAll(); ?>
            ...
            <div class="card-custom mt-4"><h5 class="text-primary">Novo Teste</h5><form method="POST" enctype="multipart/form-data"><input type="hidden" name="salvar_teste_db" value="1"><div class="row g-2"><div class="col-6"><label>Título</label><input type="text" name="titulo_teste" class="form-control" value="<?=$dados_t['titulo']??''?>" required></div><div class="col-3"><label>Segmento</label><select name="segmento" class="form-select"><option>Geral</option><option>TI</option><option>Adm</option></select></div><div class="col-3"><label>Tempo (Min)</label><input type="number" name="tempo" class="form-control" value="<?=$dados_t['tempo_limite']??60?>"></div><div class="col-12"><label>Descrição</label><textarea name="desc_teste" class="editor-rico"><?=$dados_t['descricao']??''?></textarea></div><div class="col-12"><label>Gabarito Geral</label><textarea name="gabarito_geral" class="editor-rico"><?=$dados_t['gabarito_geral']??''?></textarea></div><div class="col-6"><input type="file" name="arq_teste" class="form-control"></div><div class="col-6 text-end"><button class="btn btn-dark">Salvar</button></div></div></form></div>
            <h5 class="mt-5">Histórico</h5>
            <div class="card-custom p-0"><table class="table table-hover mb-0"><thead><tr><th>Título</th><th>Ação</th></tr></thead><tbody><?php foreach($lista as $t): ?><tr><td><?=$t['titulo']?></td><td class="text-end"><a href="?pag=banco_testes&view=<?=$t['id']?>" class="btn btn-sm btn-info text-white">Questões</a> <a href="?pag=banco_testes&edit=<?=$t['id']?>" class="btn btn-sm btn-primary">Editar</a></td></tr><?php endforeach; ?></tbody></table></div>
        <?php endif; endif; ?>
        ...
        <!-- CANDIDATOS (COM MUDANÇA DE FASE E ZAP MANUAL) -->
        ...
        ?>
            ...
            <tr>
                <td><strong><?=$c['nome']?></strong></td><td><?=$c['vaga']?></td>
                <td><span class="badge bg-secondary"><?=strtoupper($c['status'])?></span></td>
                <td><button class="btn btn-sm btn-outline-dark fw-bold btn-fase" data-appid="<?=$c['app_id']?>" data-nome="<?=$c['nome']?>" data-email="<?=$c['email']?>" data-tel="<?=preg_replace('/\D/','',$c['telefone'])?>" data-vaga="<?=$c['vaga']?>">Mudar Fase</button></td>
            ...
            <!-- AGENDAMENTO -->
            <div class="col-12 d-none p-3 bg-light border" id="box_agenda">
                <h6 class="text-warning">Agendamento</h6>
                <div class="row g-2">
                    <div class="col-4"><label>Data/Hora</label><input type="datetime-local" name="data_entrevista" id="f_data" class="form-control" onchange="updMsg()"></div>
                    <div class="col-3"><label>Tipo</label><select name="tipo_entrevista" id="f_tipo" class="form-select" onchange="updMsg()"><option>Online</option><option>Presencial</option></select></div>
                    <div class="col-5 div-online"><select name="plataforma_online" id="f_plat" class="form-select"><option>Google Meet</option><option>Teams</option></select></div>
                    <div class="col-12 div-online"><input type="text" name="link_reuniao" id="f_link" class="form-control" placeholder="Link..." onkeyup="updMsg()"></div>
                    <div class="col-3 div-pres d-none"><input type="text" id="f_cep" class="form-control" placeholder="CEP"></div>
                    <div class="col-9 div-pres d-none"><input type="text" name="end_rua" id="f_rua" class="form-control bg-light" readonly></div>
                    <div class="col-3 div-pres d-none"><input type="text" name="end_num" id="f_num" class="form-control" placeholder="Nº" onkeyup="updMsg()"></div>
                    <div class="col-4 div-pres d-none"><input type="text" name="end_comp" id="f_comp" class="form-control" placeholder="Comp" onkeyup="updMsg()"></div>
                    <div class="col-5 div-pres d-none"><input type="text" name="end_bairro" id="f_bairro" class="form-control"><input type="hidden" name="end_cidade" id="f_cidade"><input type="hidden" name="end_uf" id="f_uf"></div>
                </div>
            </div>
            
            <div class="col-12"><label>Mensagem</label><textarea name="mensagem_email" id="f_msg" class="form-control" rows="4"></textarea></div>
            <textarea name="mensagem_whats" id="f_msg_whats" class="d-none"></textarea>
            
            <div class="col-12 border-top pt-3">
                <div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="notificar_candidato" value="1" id="chk_notificar" checked><label for="chk_notificar">Notificar por E-mail</label></div>
                <div class="form-check form-switch mt-2"><input class="form-check-input" type="checkbox" name="enviar_whats" value="1" id="chk_whats"><label for="chk_whats">Popup WhatsApp (Automático)</label></div>
            </div>

            <div class="col-6"><button class="btn btn-primary w-100">SALVAR E ENVIAR</button></div>
            <div class="col-6"><button type="button" class="btn btn-success w-100 fw-bold" onclick="zapManual()"><i class="fab fa-whatsapp"></i> ENVIAR WHATSAPP MANUAL</button></div>
            ...
            <script>
                ...
                function updMsg(){
                    let st=$('#f_status').val(); let n=$('#f_nome').val(); let v=$('#f_vaga').val();
                    let txt = "";
                    if(st === 'reprovado' || st === 'standby') {
                        let mid = $('#motivo_repo').val();
                        let t = templates.find(x => x.fase == mid);
                        txt = t ? t.mensagem : "";
                    } else {
                        let t = templates.find(x => x.fase == st);
                        txt = t ? t.mensagem : "Olá {NOME}, atualização.";
                    }
                    if(st=='entrevista'){
                ...
                function zapManual(){ 
                    let tel = $('#f_tel').val(); let msg = $('#f_msg_whats').val();
                    if(tel.length < 10) alert('Telefone inválido');
                    else window.open(`https://wa.me/55${tel}?text=${encodeURIComponent(msg)}`, '_blank');
                }
